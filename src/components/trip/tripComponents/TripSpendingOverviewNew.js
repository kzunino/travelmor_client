...
const TripSpendingOverview = ({match, getTrip, trip_data}) => {
  ...
  let tripMonths = ['Entire Trip'];

  let daily_budget = parseFloat((total_budget / length).toFixed(2));

  // State to handle months if  trip is longer than 31 days
  //state should hold months in this format Aug '20
  const [months, setMonths] = useState(null);

  //make whole trip spending graph as default but with months as axis
  ...
  if (end_date) {
    ...
    // Working on printing the proper lables for months

    if (lastDayOfTrip.diff(firstDayOfTrip, 'days') > 31) {
      // calculate months
      let start = firstDayOfTrip.clone();
      ...
      <Grid item xs={12}>
        <Box m={1} boxShadow={3} className={classes.spendingChartContainer}>
          <Grid item>
            ...
            <Typography
              style={{marginTop: '.5em', color: 'white'}}
              variant='subtitle2'
            >
